    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Categorias de conteúdo</p>
        </div>
        ...
        <CardContent className="p-0">
          <div className="divide-y divide-slate-100">
            {loading ? (
              ...
              items.map((s) => (
                <div key={s.id} className="flex items-center justify-between px-5 py-3 hover:bg-muted/50">
                  <div>
                    <p className="font-medium text-foreground">{s.name}</p>
                    <p className="text-xs text-muted-foreground/80">{s._count?.videos ?? 0} vídeos</p>
                  </div>
